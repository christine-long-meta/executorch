...
def assert_only_identity_consecutive_rescales(exported_program):
    ...
    After fusion, identity pairs (composed scale ~1.0) are removed when
    R1 has no non-RESCALE users, and non-identity pairs are left
    unchanged.  Any remaining consecutive pair should therefore be
    either non-identity (intentionally preserved) or identity with a
    multi-user R1.  This assertion checks the latter case.
    """
    graph_module = exported_program.graph_module
    for node in graph_module.graph.nodes:
        if not _is_rescale(node):
...
def test_add_chain_rescale_count():
    ...
    Each add has 2 INT8 inputs (need INT8->INT32) and 1 INT32 output
    (need INT32->INT8), giving 3 RESCALEs per add = 6 total for two
    chained adds.
    """
    model = AddChain()
...
def test_fuse_consecutive_rescales():
    ...
    INT32->INT8->INT32 pairs.  The pass removes identity pairs (composed
    scale ~1.0) and leaves non-identity pairs unchanged. With random
    calibration data the composed scale is typically non-identity
    (~0.76), so most pairs are preserved.
    """
    model = AddChain()
    pipeline = PassPipeline[AddChain.input_t](
        model,
...
def test_identity_pair_with_nonzero_zp_removed():
    ...
    Same as test_identity_pair_removed but with non-zero zero points
    (zp=5/10) to verify the zero-point matching guards work correctly.
    """
    from executorch.exir.dialects._ops import ops as exir_ops
...
def test_non_identity_pair_is_preserved():
    ...
    Constructs R1(scale=0.5) → R2(scale=3.0) with composed=1.5 (non-
    identity). The pass leaves this pair unchanged because Vela cannot
    handle INT32→INT32.
    """
    from executorch.exir.dialects._ops import ops as exir_ops
...
def test_lstm_gate_pattern_fuse_correctness():
    ...
    Verifies that FuseConsecutiveRescalesPass produces correct output
    for the sigmoid→mul + tanh→mul → add chain that LSTM uses.  This
    isolates the TOSA graph correctness (no Vela/FVP needed).
    """
    model = LSTMGatePattern()
    pipeline = PassPipeline[LSTMGatePattern.input_t](
        model,
...
def test_lstm_gate_pattern_full_tosa_pipeline():
    ...
    InsertTableOpsPass, DecomposeQuantNodesPass, and all other
    downstream passes that call super().call()). This tests whether the
    optimized graph (with identity pairs removed) survives the full
    pipeline correctly.
    """
...
def test_actual_lstm_tosa_INT():
    ...
    This is the same model used by test_lstm_u55_INT but run against the
    TOSA reference model (no FVP needed). If this fails, the issue is in
    the TOSA graph generation with FuseConsecutiveRescalesPass, not
    Vela/FVP.
    """
    from executorch.backends.arm.test.tester.test_pipeline import TosaPipelineINT
...
def test_actual_lstm_u55_INT():
    ...
    This is the exact same test as test_lstm_u55_INT from
    test_lstm_arm.py.  Verifies the LSTM produces correct (non-zero)
    output on Ethos-U55 FVP after FuseConsecutiveRescalesPass removes
    identity RESCALE pairs.
    """
...
def test_lstm_gate_pattern_u55_INT():
    ...
    Smaller than the full LSTM, this tests if the specific
    sigmoid->mul->add pattern produces correct output on the Ethos-U55
    NPU after FuseConsecutiveRescalesPass removes identity RESCALE
    pairs.
    """
    from executorch.backends.arm.test.tester.test_pipeline import EthosU55PipelineINT
